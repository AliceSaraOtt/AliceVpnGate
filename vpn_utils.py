...
def get_fraud_score(ip: str) -> int | None:
    if not ip:
        return None
    import random
    import urllib.request
    ...
    with ip_cache_lock:
        cache = load_ip_cache()
        if ip in cache and "fraud_score" in cache[ip]:
            if time.time() - cache[ip].get("fraud_score_at", 0) < 30 * 24 * 3600:
                return cache[ip]["fraud_score"]
    ...
    try:
        with urllib.request.urlopen(request, timeout=10) as response:
            ...
            match = re.search(r'Fraud Score:\s*(\d+)', html, re.IGNORECASE)
            if match:
                score = int(match.group(1))
    except Exception as e:
        print(f"[get_fraud_score] Failed for {ip}: {e}", flush=True)
    ...
    if score is not None:
        with ip_cache_lock:
            ...
            if ip not in cache:
                cache[ip] = {}
            cache[ip]["fraud_score"] = score
            cache[ip]["fraud_score_at"] = time.time()
            save_ip_cache(cache)
...
def diagnose_local_obstructions(proxy_port: int = 7998, host: str = "127.0.0.1") -> tuple[int, str] | None:
    import sys
    # 1. 检查端口是否被占用
