...
echo -e "${BLUE}==========================================================${PLAIN}"
echo -e "${BLUE}        欢迎使用 AliceVPN 一键源码部署与管理脚本${PLAIN}"
echo -e "${BLUE}==========================================================${PLAIN}"

# 3. Configure GitHub Repository URL
# Default to the official repository (baoweise-bot/alice-vpngate)
DEFAULT_USER="AliceSaraOtt"
DEFAULT_REPO="AliceVpnGate"
...
# 4. Clone from GitHub
INSTALL_DIR="/opt/alicevpn"
echo -e "\n${YELLOW}[2/4] 正在从 GitHub 克隆源码到 ${INSTALL_DIR}...${PLAIN}"
if [ -d "${INSTALL_DIR}/.git" ]; then
    echo -e "  -> 检测到已存在的 Git 仓库，尝试拉取最新代码..."
    cd "${INSTALL_DIR}"
    git fetch --all
...
echo -e "${GREEN}  -> 源码部署成功！${PLAIN}"

# 5. Configure Service
echo -e "\n${YELLOW}[3/4] 正在配置系统服务...${PLAIN}"
if command -v systemctl >/dev/null 2>&1; then
    echo -e "  -> 检测到 systemd，正在创建服务配置 /lib/systemd/system/alicevpn.service ..."
    cat > /lib/systemd/system/alicevpn.service <<EOF
[Unit]
Description=AliceVPN OpenVPN Manager with HTTP/SOCKS5 Proxy
...
Restart=always
RestartSec=5
EnvironmentFile=-/etc/default/alicevpn

[Install]
WantedBy=multi-user.target
EOF
    systemctl daemon-reload
    systemctl enable alicevpn.service
elif command -v rc-service >/dev/null 2>&1; then
    echo -e "  -> 检测到 OpenRC，正在创建服务配置 /etc/init.d/alicevpn ..."
    cat > /etc/init.d/alicevpn <<EOF
#!/sbin/openrc-run
...
EOF
    chmod +x /etc/init.d/alicevpn
    rc-update add alicevpn default
else
    echo -e "${YELLOW}警告: 未能检测到 systemd 或 OpenRC，请手动管理服务。${PLAIN}"
fi

# 6. Configure global command shortcut "al"
echo -e "\n${YELLOW}[4/4] 正在创建全局命令快捷接口 'al'...${PLAIN}"
echo -e "  -> 正在写入管理脚本 /usr/bin/al ..."
cat > /usr/bin/al <<'EOF'
#!/usr/bin/env python3
import sys
import os
...
INSTALL_DIR = "/opt/alicevpn"
LOG_FILE = "/opt/alicevpn/vpngate_data/vpngate.log"
...
def load_ui_cfg():
    import json
    path = "/opt/alicevpn/vpngate_data/ui_auth.json"
    cfg = {"host": "::", "port": 9898, "secret_path": "EJsW2EeBo9lY", "password": ""}
...
def load_state():
    import json
    path = "/opt/alicevpn/vpngate_data/state.json"
...
def get_active_node_info():
    import json
    path = "/opt/alicevpn/vpngate_data/nodes.json"
    state = load_state()
...
def get_public_ip():
    path = "/opt/alicevpn/vpngate_data/public_ip.txt"
    if os.path.exists(path):
        try:
            with open(path, "r", encoding="utf-8") as f:
                ip = f.read().strip()
...
def get_service_pid(service_name="alicevpn.service"):
    try:
...
def check_service_active(service_name="alicevpn.service"):
...
def print_status():
    ...
    print_line("=======================================================")
    print_line(f"               {bold}AliceVPN 管理终端 v2.0{reset}                  ")
    print_line("=======================================================")
    print_line("【核心服务状态】")
...
def run_service_cmd(cmd):
    if shutil.which("systemctl"):
        subprocess.run(["systemctl", cmd, "alicevpn.service"])
    elif shutil.which("rc-service"):
        subprocess.run(["rc-service", "alicevpn", cmd])
    else:
        print("未检测到支持的服务管理器 (systemd/OpenRC)")

def start_service():
    print("正在启动 AliceVPN 服务...", flush=True)
    run_service_cmd("start")
    print("已发送启动指令。")
    time.sleep(1)

def stop_service():
    print("正在停止 AliceVPN 服务...", flush=True)
    run_service_cmd("stop")
    print("已发送停止指令。")
    time.sleep(1)

def restart_service():
    print("正在重启 AliceVPN 服务...", flush=True)
    run_service_cmd("restart")
    print("已发送重启指令。")
    time.sleep(1)

def show_logs():
    print("正在查看 AliceVPN 日志 (按 Ctrl+C 退出)...", flush=True)
...
def uninstall_service():
    confirm = input("确定要完全卸载 AliceVPN 吗？(y/N): ")
    if confirm.lower() == 'y':
        print("正在完全卸载 AliceVPN...", flush=True)
        stop_service()
        if shutil.which("systemctl"):
            subprocess.run(["systemctl", "disable", "alicevpn.service"])
            try:
                os.unlink("/lib/systemd/system/alicevpn.service")
            except Exception:
                pass
        elif shutil.which("rc-service"):
            subprocess.run(["rc-update", "del", "alicevpn"])
            try:
                os.unlink("/etc/init.d/alicevpn")
            except Exception:
                pass
        try:
            os.unlink("/usr/bin/al")
        except Exception:
            pass
...
def configure_port():
    ...
    while True:
        ...
        print("                      端口配置菜单                     ")
        print("=======================================================")
        print(f"1) 网页管理端口: {cfg.get('port', 9898)}")
        print(f"2) 代理出站端口: {cfg.get('proxy_port', 7998)}")
        print("3) 返回主菜单")
        print("-------------------------------------------------------")
...
def get_status_state():
    ...
    return (
        ...
        check_service_active("alicevpn.service"),
        check_openvpn_process(),
        get_service_pid("alicevpn.service")
    )
...
def main():
    ...
    options = {
        '1': ("启动服务 (al start)", start_service),
        '2': ("停止服务 (al stop)", stop_service),
        '3': ("重启服务 (al restart)", restart_service),
        '4': ("日志监控 (al logs)", show_logs),
        '5': ("网页配置 (al web)", configure_web),
        '6': ("端口配置 (al port)", configure_port),
        '7': ("账号密码 (al password)", configure_credentials),
        '8': ("一键更新 (al update)", update_service),
        '9': ("完全卸载 (al uninstall)", uninstall_service),
        '0': ("退出终端", None)
    }
...
")
    ...
    if [[ "$is_custom" =~ ^[Yy]$ ]]; then
        ...
        # 1. Custom port
        while true; do
            read -p "请输入自定义管理端口 [1-65535, 默认 9898]: " input_port
...
if [ -d "/etc/sysctl.d" ]; then
    cat > /etc/sysctl.d/99-alicevpn.conf <<EOF
net.ipv4.conf.all.rp_filter = 2
net.ipv4.conf.default.rp_filter = 2
EOF
    sysctl -p /etc/sysctl.d/99-alicevpn.conf >/dev/null 2>&1 || true
else
...
echo -e "\n正在启动 AliceVPN 服务并初始化网络..."
if command -v systemctl >/dev/null 2>&1; then
    systemctl restart alicevpn.service || true
elif command -v rc-service >/dev/null 2>&1; then
    rc-service alicevpn restart || true
fi

# Wait and poll for node loading and active connection
echo -e "\n正在等待 AliceVPN 首次获取节点并建立加密通道 (此过程可能需要 5-30 秒)..."
ACTIVE_ID=""
LAST_MSG=""
for i in {1..90}; do
...
if [ -f "$AUTH_FILE" ]; then
    ...
    UI_PORT=$(python3 -c "import json; print(json.load(open('$AUTH_FILE')).get('port', 9898))" 2>/dev/null || echo "9898")
fi
...
echo -e "\n${GREEN}==========================================================${PLAIN}"
echo -e "${GREEN}             AliceVPN 源码一键部署已完成！${PLAIN}"
echo -e "${GREEN}==========================================================${PLAIN}"
...
echo -e "  * HTTP/SOCKS5 代理端口:  ${BLUE}http://127.0.0.1:7998/${PLAIN}  或  ${BLUE}http://[::1]:7998/${PLAIN}"
echo -e " --------------------------------------------------------"
echo -e "  * 快速状态指令:   ${YELLOW}al status${PLAIN}  或  ${YELLOW}al${PLAIN}"
echo -e "  * 查看实时日志:   ${YELLOW}al logs${PLAIN}"
echo -e "  * 停止服务:       ${YELLOW}al stop${PLAIN}"
echo -e "  * 重启服务:       ${YELLOW}al restart${PLAIN}"
echo -e "=========================================================="
echo
